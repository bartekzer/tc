...
impl TypeChecker {
    ...
    pub fn check(&mut self, expr: Expression) -> Result<Type, Error> {
        match expr {
            ...
            Expression::Addition { lhs, rhs, .. } => {
                let lhs_type = self.check(*lhs.clone())?;
                if Type::Int == lhs_type {
                    let rhs_type = self.check(*rhs.clone())?;
